...
import { IGhlibliFilm } from '../models/ghibli-film.model';
import { GhibliApiService } from '../services/ghibli-api.service';

@Component({
  selector: 'app-ghibli-films',
  templateUrl: './ghibli-films.component.html',
  styleUrls: ['./ghibli-films.component.scss']
})
export class GhibliFilmsComponent implements OnInit {

   films: IGhlibliFilm[] = [];

  constructor( private dataGhibleService: GhibliApiService ) { }

  ngOnInit(): void {
    this.dataGhibleService.getAllFilms().subscribe((ghibliFilms) => {
      this.films = ghibliFilms;
    });
  }
